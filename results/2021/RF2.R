...
# Simple RF using 11 landscape covariates from MLR
# Simple GW-RF using 11 sig landscape covariates from MLR
...
predictors <- c("SLOPE", "h2oLakesPe", "h2oRdDens", "h2oHiCascP", "h2oWetland", 
                "Shrub21", "BurnRCA", "AgBuf", "WetlandBuf", "HiCascBuf", "RoadsBuf")
...
print(rf_formula)
# Fit basic RF model - 500 as std # of trees, sqrt(11) = 3 for regression, calculate varb importance 
rf_model <- randomForest(formula = rf_formula, data = train_data, ntree = 500, mtry = 3, importance = TRUE)
# Print model summary
print(rf_model)
...
gwrf_model <- grf( formula = rf_formula, dframe = train_data, bw = 50, ntree = 500, mtry = 3, kernel = "adaptive", coords = train_coords)
...
comparison <- data.frame(Model = c("Basic RF 2", "GW-RF 2"),RMSE = c(round(rf_rmse, 4), round(gwrf_rmse, 4)),MAE = c(round(rf_mae, 4), round(gwrf_mae, 4)),R_squared = c(round(rf_r2, 4), round(gwrf_r2, 4)))
print(comparison)
